...
var DEFAULTS={bbLen:20,bbMult:2,sqLook:126,sqPct:0.20,volMult:1.5,atrLen:14,target:1.0,horizon:5,cooldown:5,reqSqueeze:false,reqVol:false,reqTrend:false,fill:'close',slippageBps:0,regime:false};
var DATA={}, SETTINGS=loadSettings(), JOURNAL=[], FILTER={sym:null,res:null,dir:null};

function loadSettings(){ try{ var s=JSON.parse(localStorage.getItem('bbv_settings')); if(s) return Object.assign({},DEFAULTS,s);}catch(e){} return Object.assign({},DEFAULTS); }
function saveSettings(){ localStorage.setItem('bbv_settings', JSON.stringify(SETTINGS)); }
...
function renderExplainer(){
  document.getElementById('rules-explainer').textContent =
    'A signal fires when the close crosses outside the Bollinger band ('+SETTINGS.bbLen+','+SETTINGS.bbMult+')'+
    ...
    '. Entry is the '+(SETTINGS.fill==='open'?'open of the bar after the signal':'signal-day close')+(SETTINGS.slippageBps?' plus '+SETTINGS.slippageBps+'bps slippage':'')+(SETTINGS.regime?', only with the 200-day average on the signal side':'')+'. A HIT means price moved +'+SETTINGS.target+'× ATR('+SETTINGS.atrLen+') in the signal direction within '+SETTINGS.horizon+' bars before moving the same distance against it; if both levels trade in one bar it scores a MISS (conservative). If neither level trades within '+SETTINGS.horizon+' bars, the close-to-close direction decides. Batting average = hits ÷ signals. Signals repeat only after '+SETTINGS.cooldown+' bars.';
}
...
function settingsToUI(){
  var m={bbLen:'s-bblen',bbMult:'s-bbmult',sqLook:'s-sqlook',sqPct:'s-sqpct',volMult:'s-volmult',atrLen:'s-atrlen',target:'s-target',horizon:'s-horizon',cooldown:'s-cooldown',slippageBps:'s-slip'};
...
document.getElementById('export-csv').onclick=function(){
  var out=['date,symbol,direction,entry,score,squeeze,volume_confirm,trend_align,result,bars_to_resolve,mfe,mae,atr'];
  JOURNAL.forEach(g=> out.push([g.date,g.sym,g.dir===1?'up':'down',g.entry,g.score,g.squeeze,g.vol,g.trend,g.res,g.bars,g.mfe,g.mae,g.atr].join(',')));
...
function renderSweep(){
  ...
  var targets=[0.5,1.0,1.5,2.0], horizons=[3,5,7,10];
  var html='<table><thead><tr><th class="sweep-head">target \\ horizon</th>';
  horizons.forEach(function(h){ html+='<th class="sweep-head">'+h+' bars</th>'; });
  html+='</tr></thead><tbody>';
  targets.forEach(function(t){
    html+='<tr><td class="sweep-head">'+t+'x ATR</td>';
    horizons.forEach(function(hz){
      var n=0,hh=0;
      Object.keys(DATA).forEach(function(s){
        var sigs=bbAnalyze(DATA[s], Object.assign({},SETTINGS,{target:t,horizon:hz}));
        n+=sigs.length; hh+=sigs.filter(function(g){return g.res==='hit';}).length;
      });
      var ba=n?100*hh/n:0;
      var col=n<30?'rgba(139,148,158,0.15)':(ba>=60?'rgba(63,185,80,0.35)':(ba>=50?'rgba(63,185,80,0.15)':'rgba(248,81,73,0.12)'));
      html+='<td class="sweep-cell" style="background:'+col+'">'+(n?ba.toFixed(1)+'%':'–')+'<span class="sweep-n">n='+n+'</span></td>';
    });
    html+='</tr>';
  });
  el.innerHTML=html+'</tbody></table><p class="muted">Green = at or above the 60% graduation floor. Gray = fewer than 30 signals, too thin to read. Same rules as Settings, varying only target and horizon.</p>';
}
function renderFolds(){
  ...
  var html='<table><thead><tr><th>Period</th><th>Signals</th><th>Win %</th><th>95% CI</th></tr></thead><tbody>';
  folds.forEach(function(f,k){
    if(!f.length){ html+='<tr><td>fold '+(k+1)+'</td><td>0</td><td>–</td><td>–</td></tr>'; return; }
    ...
    html+='<tr><td>'+fd[0]+' → '+fd[fd.length-1]+'</td><td>'+f.length+'</td><td class="'+((hh/f.length)>=0.5?'pos':'neg')+'">'+(100*hh/f.length).toFixed(1)+'%</td><td>'+(ci?ci.lo+'–'+ci.hi+'%':'–')+'</td></tr>';
  });
