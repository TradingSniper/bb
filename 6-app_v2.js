...
function renderSymbols(){
  ...
  var rows=Object.keys(map).map(function(sym){ var m=map[sym]; return {sym:sym, n:m.n, h:m.h, miss:m.n-m.h, win:m.h/m.n, avg:m.score/m.n}; });
  ...
  rows.forEach(function(r){
    var tr=document.createElement('tr');
    tr.innerHTML='<td>'+r.sym+'</td><td>'+r.n+'</td><td>'+r.h+'</td><td>'+r.miss+'</td><td class="'+(r.win>=0.5?'pos':'neg')+'">'+pct(r.h,r.n)+'</td><td>'+r.avg.toFixed(0)+'</td>';
    tr.onclick=function(){ FILTER.sym=FILTER.sym===r.sym?null:r.sym; render(); switchTab('journal'); };
    tb.appendChild(tr);
  });
  var keys=['sym','n','h','miss','win','avg'], labels=['Symbol','Signals','Hits','Misses','Win %','Avg score'];
  document.querySelectorAll('#symbol-table thead th').forEach(function(th,i){
    th.style.cursor='pointer';
    th.textContent = labels[i] + (k===keys[i] ? (d===1?' \u25b2':' \u25bc') : '');
    th.onclick=function(){ if(SYM_SORT.key===keys[i]) SYM_SORT.dir*=-1; else { SYM_SORT.key=keys[i]; SYM_SORT.dir=(keys[i]==='sym'?1:-1); } renderSymbols(); };
  });
}
...
function renderUniverse(){
  ...
  el.textContent = syms.length+' symbols x 10 years of daily bars, in the six sectors picked on pre-2016 data only (clean selection, verified on data they were never chosen on). '+parts.join('  |  ');
}
