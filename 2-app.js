...
var DEFAULTS={bbLen:20,bbMult:2,sqLook:126,sqPct:0.20,volMult:1.5,atrLen:14,target:1.0,stop:1.0,horizon:5,cooldown:5,dir:1,reqSqueeze:false,reqVol:false,reqTrend:false,fill:'close',slippageBps:10,regime:true};
var DATA={}, SETTINGS=loadSettings(), JOURNAL=[], FILTER={sym:null,res:null,dir:null};

function loadSettings(){ try{ var s=JSON.parse(localStorage.getItem('bbv_settings_v3')); if(s) return Object.assign({},DEFAULTS,s);}catch(e){} return Object.assign({},DEFAULTS); }
function saveSettings(){ localStorage.setItem('bbv_settings_v3', JSON.stringify(SETTINGS)); }
...
function renderSweep(){
  ...
  el.innerHTML=html+'</tbody></table><p class="muted">Green = at or above the 60% graduation floor with positive avg R. Gray = fewer than 30 signals, too thin to read. Same rules as Settings (stop stays '+SETTINGS.stop+'\u00d7 ATR), varying only target and horizon. A high win % with negative avg R loses money - read both numbers together.</p>';
}
